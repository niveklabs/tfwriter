...
```terraform
resource "panos_panorama_edl" "this" {
  # certificate_profile - (optional) is a type of string
...
output "id" {
  ...
  value       = panos_panorama_edl.this.id
}

output "password_enc" {
  description = "returns a string"
  value       = panos_panorama_edl.this.password_enc
  sensitive   = true
}

output "this" {
  value = panos_panorama_edl.this
}
```
